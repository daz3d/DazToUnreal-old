...
bool FDazToUnrealModule::Tick(float DeltaTime)
{
	...
	if (ServerSocket->HasPendingData(BytesPending))
	{
		...
		if (ServerSocket->RecvFrom(Data, sizeof(Data), BytesRead, *Sender))
		{
			...
			FString Message = ANSI_TO_TCHAR(CharData);
			...
			// Start the import
			ImportFromDaz(FBXPath, AssetName, AssetTypeEnum, ImportFolder);
		}
	}
	return true;
}
...
UObject* FDazToUnrealModule::ImportFromDaz(const FString& FBXPath, const FString& AssetName, const DazAssetType& AssetType, const FString& ImportFolder)
{
	...
	FString SubdivisionFileName = ImportCharacterFolder / AssetName + TEXT("_Subdivisions.txt");
	if (FPaths::FileExists(SubdivisionFileName))
	{
		TArray<FString> SubdivisionFileLines;
		FFileHelper::LoadFileToStringArray(SubdivisionFileLines, *SubdivisionFileName);
		for (FString SubdivisionFileLine : SubdivisionFileLines)
		{
			if (SubdivisionFileLine.StartsWith(TEXT("Version"))) continue;
			TArray<FString> Parts;
			SubdivisionFileLine.ParseIntoArray(Parts, TEXT(", "), false);
			if (Parts.Num() == 3)
			{
				SubdivisionLevels.Add(Parts[1], FCString::Atoi(*Parts[2]));
			}
		}
	}

	// Use the maps file to find the textures to load
	TArray<FString> TextureFileLines;
	FString TextureListFileName = ImportCharacterFolder / AssetName + TEXT("_Maps.txt");
	FFileHelper::LoadFileToStringArray(TextureFileLines, *TextureListFileName);
	TMap<FString, FString> TextureFileSourceToTarget;
	TArray<FString> IntermediateMaterials;

	for (FString TextureFileLine : TextureFileLines)
	{
		if (TextureFileLine.StartsWith(TEXT("Version"))) continue;
		TArray<FString> Parts;
		TextureFileLine.ParseIntoArray(Parts, TEXT(", "), false);
		if (Parts.Num() >= 6)
		{
			...
			if (Version == TEXT("1"))
			{
				FString MaterialName = AssetName + TEXT("_") + Parts[1];
				MaterialName = FDazToUnrealUtils::SanitizeName(MaterialName);
				FString TextureType = Parts[2];
				...
				if (!TextureName.IsEmpty())
				{
					...
					TextureProperty.Name = Parts[2] + TEXT(" Texture");
					...
					SwitchProperty.Name = Parts[2] + TEXT(" Texture Active");
			...
			if (Version == TEXT("2"))
			{
				FString ObjectName = Parts[1];
				ObjectName = FDazToUnrealUtils::SanitizeName(ObjectName);
				IntermediateMaterials.AddUnique(ObjectName + TEXT("_BaseMat"));
				FString ShaderName = Parts[3];
				FString MaterialName = AssetName + TEXT("_") + Parts[2];
				MaterialName = FDazToUnrealUtils::SanitizeName(MaterialName);
				FString TextureType = Parts[4];
				...
				Property.Type = Parts[6];
				Property.Value = Parts[5];
				...
				if (!TextureName.IsEmpty())
				{
					// If a texture is attached add a texture property
					FDUFTextureProperty TextureProperty;
					TextureProperty.Name = Parts[4] + TEXT(" Texture");
					TextureProperty.Type = TEXT("Texture");
					...
					// and a switch property for things like Specular that could come from different channels
					FDUFTextureProperty SwitchProperty;
					SwitchProperty.Name = Parts[4] + TEXT(" Texture Active");
					SwitchProperty.Type = TEXT("Switch");
					SwitchProperty.Value = TEXT("true");
	...
	TArray<FString> MorphFileLines;
	FString MorphListFileName = ImportCharacterFolder / AssetName + TEXT("_MorphNames.txt");
	TMap<FString, FString> MorphMappings;
	FFileHelper::LoadFileToStringArray(MorphFileLines, *MorphListFileName);
	for (FString line : MorphFileLines)
	{
		if (line.Contains(TEXT("=")))
		{
			FString MorphOriginalName;
			FString MorphNewName;
			line.Split(TEXT("="), &MorphOriginalName, &MorphNewName);
			MorphMappings.Add(MorphOriginalName, MorphNewName);
		}
	}
